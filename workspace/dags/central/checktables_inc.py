...
with DAG(dag_id, default_args=default_args, schedule_interval='15 18 * * *', catchup=False,
         tags=['main'], params={'dt_inc_begin': None, 'dt_inc_end': None}) as dag:

    t_truncate = run_sql(script='/checktables/checktables_truncate.sql', task_id='truncate_stg')
    t_finish_load = finish_load()
    t_get_load_params = get_load_params()
    t_load_ods = run_sql(script='/checktables/ods_checktables.sql', task_id='fill_ods')

    t_get_load_params >> get_inc(task_id='get_load_inc') >> t_truncate >> t_extract_sql('checktables') >> t_load_ods >> t_finish_load
