...
with DAG(dag_id, default_args=default_args, schedule_interval='30 18 * * *', catchup=False, tags=['main','central']) as dag:

    t_truncate = run_sql(
        script='inventories/truncate.sql',
   ...
   # t_load_mart = run_sql(script='mart_total_results_by_store.sql', task_id='load_mart')
    ...
    t_get_load_params >> t_truncate >> [t_extract_sql('consignments')] >> t_finish_load
