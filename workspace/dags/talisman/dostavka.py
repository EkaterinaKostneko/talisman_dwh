...
from talisman.helpers import (run_sql, get_load_id, finish_load, extract_sql, run_python, get_load_params, step)
...
with DAG(dag_id, default_args=default_args, schedule_interval='50 19 * * *', catchup=False, tags=['main']) as dag:
    ...
    t_truncate = run_sql(script='truncate_delivery.sql', task_id='truncate_stg')
    t_load_mart = step('load_mart')
    t_load_mart_goods = run_sql(script='mart_delivery_goods.sql', task_id='load_goods')
    t_load_mart_couriers = run_sql(script='mart_delivery_couriers.sql', task_id='load_couriers')
    t_load_mart_areas = run_sql(script='mart_delivery_areas.sql', task_id='load_areas')
