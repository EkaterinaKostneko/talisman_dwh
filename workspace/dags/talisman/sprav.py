...
from talisman.helpers import (run_sql, finish_load, extract_sql, run_python, get_load_params)
...
with DAG(dag_id, default_args=default_args, schedule_interval='0 20 * * *', catchup=False, tags=['main']) as dag:
    ...
    t_load_mart = run_sql(script='mart_sprav.sql', task_id='load_mart')
    t_finish_load = finish_load()
    # t_get_load_id = get_load_id()
    t_get_load_params = get_load_params()

    t_get_load_params >> t_truncate >> [t_extract_sql('sc135_storage'), t_extract_sql('sc133_contragent'), t_extract_sql('_1sconst')] >> t_load_mart >> t_finish_load
