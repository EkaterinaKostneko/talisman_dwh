...
default_args = {
    'owner'                 : 'airflow',
    'description'           : 'SQL Move Firebird PG',
...
dag_id = 'df_test_sql_move_firebird_pg'
...
with DAG(dag_id, default_args=default_args, schedule_interval=None, catchup=False, tags=['test']) as dag:
    t1 = DockerOperator(
        ...
        environment={**environment,
                     **{'AF_DWH_DB_CONNECTION_PRODUCER': serialize_connection('firebird_341'),
                        'AF_DWH_DB_CONNECTION_CONSUMER': serialize_connection('testpostgresql'),
                        'AF_PRODUCER_SCRIPT_PATH': '/app/ws/source/df/sql/scripts/tests/sql_move_producer_firebird_pg.sql',
